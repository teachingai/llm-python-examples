...
class NLU:
    ...
    def _get_completion(self, prompt, model="glm-3-turbo"):
        ...
        # print(markdown_text)
        plain_json = extract_json_from_markdown(markdown_text)
        # print(plain_text[0])
        semantics = json.loads(plain_json[0])
        return {k: v for k, v in semantics.items() if v}

    def parse(self, user_input):
        prompt = self.prompt_template.replace("__INPUT__", user_input)
        return self._get_completion(prompt, model="glm-4-flash")
...
class DialogManager:
    ...
    def _call_chatgpt(self, prompt, model="glm-3-turbo"):
    ...
    def run(self, user_input):
        ...
        # 调用chatgpt获得回复
        response = self._call_chatgpt(prompt_for_chatgpt, model="glm-4-flash")
